...
public class CombatTask {
        ...
        new Thread(() -> {
            while (running) {
                try {
                    ...
                    if (mc.thePlayer == null) { sleep(1000); continue; }
                    // Find nearest mob
                    List<Entity> mobs = mc.theWorld.loadedEntityList;
                    Entity target = null;
                    double minDist = 6.0;
                    for (Entity e : mobs) {
                        if (e instanceof EntityMob && e.getDistanceToEntity(mc.thePlayer) < minDist) {
                            target = e;
                    ...
                    if (mc.thePlayer.getHealth() < 6.0f) {
                        for (int i=0; i<9; i++) {
                            ...
                            if (stack != null && stack.getItem() instanceof ItemFood) {
                                ...
                                mc.playerController.rightClick(mc.thePlayer, mc.theWorld);
    ...
    private float getAngleTo(Entity e) {
        double dx = e.posX - Minecraft.getMinecraft().thePlayer.posX;
        double dz = e.posZ - Minecraft.getMinecraft().thePlayer.posZ;
