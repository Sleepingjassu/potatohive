        ...
        worker = new Thread(() -> {
            while (running) {
                try {
                    ...
                    // Break crop in front
                    BlockPos front = mc.thePlayer.getPosition().offset(mc.thePlayer.getHorizontalFacing(), 1);
                    mc.playerController.clickBlock(front, EnumFacing.UP);
                    sleep(gaussian(300));
                    // Place seed (assume seed in hotbar slot 1)
                    int oldSlot = mc.thePlayer.inventory.currentItem;
                    mc.thePlayer.inventory.currentItem = 0;
                    mc.playerController.rightClickBlock(mc.thePlayer, mc.theWorld, front, EnumFacing.UP, new float[]{0.5f,0.5f,0.5f});
